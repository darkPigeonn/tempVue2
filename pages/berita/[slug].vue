...
const endpoint = 'https://api.imavi.org/imavi/news/view/' + slug;
 ...
 * */
const { data: beritaDetail } = await useFetch(endpoint, {
  headers: {
...
<template>
  <v-container>
    <div style="margin-top: 100px" class="ptserrifText">
      <h1 class="text-h4">{{ beritaDetail.title }}</h1>
      <div class="text-end">
        <p style="margin-top: 12px; margin-bottom: 12px">{{ beritaDetail.publishDate }}</p>
        <p>Oleh: {{ beritaDetail.author }}</p>
      </div>
      <p v-html="beritaDetail.content"></p>
    </div>
    <div class="text-center mt-4">
      <NuxtLink to="/berita"><v-btn>Kembali</v-btn></NuxtLink>
    </div>
  </v-container>
</template>
